# -*- coding: utf-8 -*-
"""Brats_MaxViT_ReviewII_first10.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1gFLL8r30KZYW7r-Fdk_5E2YQZcBlXKWa
"""

# ============================================================
# 1. Install required libraries
# ============================================================
!pip install -q segmentation-models-pytorch==0.3.3 timm nibabel opencv-python-headless scikit-learn tqdm pandas
...
# ============================================================
CONFIG = {
    "IMG_SIZE": 224,            # maxvit_tiny_tf_224 is pretrained at 224x224 -- keep this size
    "NUM_CLASSES": 4,           # background, necrotic core, edema, enhancing tumor
    "BATCH_SIZE": 4,            # configurable batch size
    "EPOCHS": 10,               # configurable number of epochs
    "LEARNING_RATE": 1e-4,
    "WEIGHT_DECAY": 1e-5,
    "EARLY_STOP_PATIENCE": 7,
    "EDGE_SLICES_SKIP": 30,     # skip the first 30 and last 30 axial slices of every volume
    "TRAIN_RATIO": 0.9,
    "TEST_RATIO": 0.1,
    "NUM_WORKERS": 2,
...
# ============================================================
# 8. Build a slice-level index (2D axial slices, excluding edge slices)
# ============================================================
# Instead of keeping only slices with a minimum amount of tumor content,
# we keep every axial slice EXCEPT the first EDGE_SLICES_SKIP and last
# EDGE_SLICES_SKIP slices of each patient's volume (these edge slices are
# mostly background/air and carry little useful brain anatomy).
# We only read the NIfTI header (.shape) to get the number of slices, so
# the full segmentation volume is never loaded into memory here.
def build_slice_index(patient_dirs, edge_skip=30):
    ...
    for d in tqdm(patient_dirs, desc="Indexing slices"):
        ...
        try:
            n_slices = nib.load(seg_path).shape[2]   # header-only read, no voxel data loaded
        except Exception as e:
            print(f"Warning: could not read header of {seg_path} ({e}). Skipping patient.")
            continue
    ...
    if skipped_short:
        print(f"Skipped {skipped_short} patient(s) with fewer than {2 * edge_skip} slices.")
    return samples
...
# ============================================================
# 13. Build U-Net++ model (MaxViT Transformer/CNN hybrid encoder, ImageNet pretrained)
# ============================================================
# MaxViT keeps a standard CNN-style stem (downsamples by 2x first, like
# resnet), so it produces all 5 feature-map levels UnetPlusPlus's nested
# decoder expects (strides 2, 4, 8, 16, 32). Unlike Swin/mit_b0 (which
# jump straight to a 4x-downsampling patch stem and skip the 2x level),
# this does NOT crash with a decoder channel/size mismatch.
# It also accepts 4 input channels natively via timm -- no widening trick needed.
model = smp.UnetPlusPlus(
    encoder_name="tu-maxvit_tiny_tf_224",
...
print(f"Model created: U-Net++ (MaxViT-Tiny encoder). Total parameters: {num_params:,}")
...
def compute_batch_metrics(logits, targets, num_classes):
    ...
    enhancing_tumor_dice = dice_coefficient(preds, targets, class_ids=[3])
    ...
    return {
        ...
        "enhancing_tumor_dice": enhancing_tumor_dice,
        "tumor_core_dice": tumor_core_dice,
    }

# ============================================================
# 16. Optimizer, Scheduler, AMP scaler, Early Stopping
# ============================================================
...
# EarlyStopping: stops training when training loss stops improving.
class EarlyStopping:
...
METRIC_KEYS = [
    ...
    "background_dice", "edema_dice", "enhancing_tumor_dice", "tumor_core_dice",
]
...
# ============================================================
# 18. Main training loop (with checkpointing & early stopping)
# ============================================================
history = {
    "epoch": [], "train_loss": [], "train_accuracy": [],
    "train_dice": [], "train_background_dice": [], "train_edema_dice": [],
    "train_enhancing_tumor_dice": [], "train_tumor_core_dice": [],
    "lr": [], "epoch_time_sec": [],
}
...
for epoch in range(1, CONFIG["EPOCHS"] + 1):
    ...
    history["train_enhancing_tumor_dice"].append(train_metrics["enhancing_tumor_dice"])
    ...
    print(
        ...
        f"Enhancing Tumor Dice: {train_metrics['enhancing_tumor_dice']:.4f} | "
    ...
    if early_stopping.step(train_loss):
        print(f"\nEarly stopping triggered at epoch {epoch} (no improvement in train loss for "
...
# Per-region Dice curves (Background / Edema / Enhancing Tumor / Tumor Core)
fig2, ax = plt.subplots(1, 1, figsize=(8, 5))
ax.plot(history_df["epoch"], history_df["train_background_dice"], label="Background")
ax.plot(history_df["epoch"], history_df["train_edema_dice"], label="Edema")
ax.plot(history_df["epoch"], history_df["train_enhancing_tumor_dice"], label="Enhancing Tumor")
ax.plot(history_df["epoch"], history_df["train_tumor_core_dice"], label="Tumor Core")
ax.set_xlabel("Epoch"); ax.set_ylabel("Dice Score"); ax.set_title("Per-Region Dice Curves")
ax.legend(); ax.grid(True)
plt.tight_layout()
plt.savefig("/content/per_region_dice_curves.png", dpi=150)
plt.show()
...
print(f"Enhancing Tumor Accuracy:  {test_metrics['enhancing_tumor_dice']:.4f}")
